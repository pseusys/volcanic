...
class SkinnedCylinder(KeyFrameControlNode):
    ...
    def __init__(self, shader, sections=11, quarters=20):
        ...
        bone_offsets = [identity(), identity(), translate(sections/2-1, 0,0)]
...
class SkinnedCylinder2(KeyFrameControlNode):
    """ Deformable cylinder """
    def __init__(self, shader, sections=11, quarters=20):
        forearm = KeyFrameControlNode(
            {0: (0, 0, 0)},
        ...
        # this "arm" node and its transform serves as control node for bone 0
        # we give it the default identity keyframe transform, doesn't move
        super().__init__({0: (-11, 0, 0)}, {0: quaternion()}, {0: 1}, rotate((1,0,0), 180.))
        self.add(forearm)
        forearm.add(hand)
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        left = SkinnedCylinder2(shader)
        right = SkinnedCylinder(shader)
        
        viewer.add(left,right)
    else:
